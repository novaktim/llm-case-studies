...
def generate_best_model_script(best_model_name, X_train, y_train, pdf_context):
    ...
    """
    prompt = (
    f"Write a Python function that implements the best model ({best_model_name}). "
    "The function should:\n"
    "1. Take train and test datasets as input.\n"
    "2. Train the best model on the train dataset.\n"
    "3. Output predictions for the test dataset.\n"
    "4. Save the trained model to a file.\n\n"
    "The function should be named `train_and_predict` and should have the following structure:\n"
    "```python\n"
    "def train_and_predict(train_data, test_data, target_column, model_save_path='best_model.pkl'):\n"
    "    '''\n"
    "    Train the best model on the train dataset and generate predictions for the test dataset.\n\n"
    "    Parameters:\n"
    "    - train_data: Training dataset (pandas DataFrame).\n"
    "    - test_data: Test dataset (pandas DataFrame).\n"
    "    - target_column: Name of the target column in the training dataset.\n"
    "    - model_save_path: Path to save the trained model (default: 'best_model.pkl').\n\n"
    "    Returns:\n"
    "    - y_pred: Predictions for the test dataset.\n"
    "    '''\n"
    "    # Import necessary libraries\n"
    "    import joblib\n"
    f"    from sklearn.ensemble import {best_model_name}\n\n"
    "    # Separate features and target\n"
    "    X_train = train_data.drop(columns=[target_column])\n"
    "    y_train = train_data[target_column]\n"
    "    X_test = test_data.drop(columns=[target_column])\n\n"
    "    # Initialize the best model\n"
    f"    model = {best_model_name}(random_state=42)  # Replace with appropriate hyperparameters if needed\n\n"
    "    # Train the model\n"
    ...
    "1. The function is complete, executable, and properly formatted.\n"
    "2. Use consistent indentation (4 spaces per level).\n"
    "3. Ensure all brackets (`[`, `{`, `(`) are properly closed.\n"
    "4. Avoid unexpected indentation or misaligned code blocks.\n"
    "5. Generate complete and executable code without truncation or omissions.\n"
    "6. DO NOT use an external dataset URL. Use the provided preprocessed data."
...
def save_script_to_json(script_content, json_file_path):
    """
    Save the generated script to a JSON file and make it callable.

    Parameters:
    - script_content: The Python script to save (string).
    - json_file_path: Path to save the JSON file.

    Returns:
    - The callable function (if successful).
    """
    try:
        ...
        # Step 2: Make the function callable
        exec(script_content, globals())  # Define the function in the current namespace
        print("✅ Function saved as callable.")

        # Return the callable function
        return globals().get("train_and_predict")  # Replace "train_and_predict" with the function name
    except Exception as e:
        print(f"❌ Error: {e}")
        return None
